...
public class CalculatorController {
    ...
    //
       double result = -1.0;
        ...
        return ApiResponse.builder() //
          .x(x) //
          .y(y) //
          .operation(request.getOperation().toString().toLowerCase())
          .result(String.valueOf(result)) //
          .build();
  }
  ...
  public static double calculate(double d1, double d2, Operation operation){
    return switch(operation){
      ...
      case MUL -> BigDecimal.valueOf(d1).multiply(BigDecimal.valueOf(d2)).setScale(5).doubleValue();
      case DIV -> BigDecimal.valueOf(d1).divide(BigDecimal.valueOf(d2)), 5, 
